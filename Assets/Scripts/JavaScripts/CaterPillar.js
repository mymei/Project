...
var targetAimPos : Vector3;
var turretTraverse : float = 10;
...
function Update()
{
	var lookRotation = Quaternion.LookRotation(targetAimPos - transform.Find("Joint").transform.position);
	transform.Find("Joint").transform.rotation = Quaternion.Slerp(transform.Find("Joint").transform.rotation, lookRotation, Time.deltaTime * turretTraverse);
	
//	transform.Find("Joint").transform.rotation = Quaternion.Slerp(transform.Find("Joint").transform.rotation, tmpTrans.rotation, Time.deltaTime * 0.0001);	
...
function ApplyThrottle(canDrive : boolean, relativeVelocity : Vector3)
{
	for(var w : CaterpillarWheel in wheels)
	{
		...
		w.collider.motorTorque = flag?multiplier * 500:0;
		w.collider.brakeTorque = !flag?500 * Mathf.Abs(throttle):0;
	}	
}
